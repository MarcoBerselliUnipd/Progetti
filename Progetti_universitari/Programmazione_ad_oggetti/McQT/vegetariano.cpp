...
Vegetariano::Vegetariano(int q):Panino("Vegetariano",q){
    Prodotto p("Pane",0.5,1);
    Prodotto m("Mozzarella",0.6,1);
    Prodotto i("Insalata",0.2,1);
    Prodotto po("Pomodoro",0.2,1);
    Panino::aggiungiProdotto(po);
    Panino::aggiungiProdotto(i);
    Panino::aggiungiProdotto(m);
    Panino::aggiungiProdotto(p);
}

void Vegetariano::aggiungiProdotto(const Prodotto & prod){
    Prodotto i("Insalata",0.2,1);
    Prodotto po("Pomodoro",0.2,1);
    Prodotto m("Mozzarella",0.6,1);
    bool trovato=false;
    if(prod==i || prod== po || prod==m){
...
void Vegetariano::rimuoviProdotto(const Prodotto & prod){
    Prodotto p("Pane",0.5,1);
    Prodotto po("Pomodoro",0.2,1);
    Prodotto m("Mozzarella",0.6,1);
...
double Vegetariano::prezzoAlPezzo() const{
    return Panino::prezzoAlPezzo()-0.1*Panino::prezzoAlPezzo();
}


std::ostream& operator<<(std::ostream& os, const Vegetariano& p){
